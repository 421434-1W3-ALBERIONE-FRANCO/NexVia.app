...
export default function Home() {
  ...
  useEffect(() => {
    ...
    nexvia.entities.Camion.get(camionId).then(setCamionAsignado).catch(() => {});
    const unsub = nexvia.entities.Camion.subscribe((event) => {
  ...
  const cargarDatos = async () => {
    try {
      const [camionesData, configsData, misViajes] = await Promise.all([
        nexvia.entities.Camion.filter({ estado: 'disponible' }),
        nexvia.entities.Configuracion.list(),
        user ? nexvia.entities.Viaje.filter({ usuario_id: user.id }, '-created_date', 5) : [],
      ]);
  ...
  const handleSolicitar = async () => {
    ...
    setSolicitando(true);
    try {
      const viaje = await nexvia.entities.Viaje.create({
  ...
  const handleCancelar = async () => {
    if (!viajeActual) return;
    await nexvia.entities.Viaje.update(viajeActual.id, { estado: 'cancelado' });
    limpiarViaje();
  };
