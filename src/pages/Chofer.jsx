...
export default function Chofer() {
  ...
  useEffect(() => {
    ...
    const updatePosition = async (pos) => {
      ...
      base44.entities.Camion.update(miCamion.id, { lat: latitude, lng: longitude }).catch(() => {});
      ...
      if (viaje.estado === 'aceptado') {
        ...
        if (dist <= UMBRAL_LLEGADA_KM) {
          ...
          await base44.entities.Viaje.update(viaje.id, { estado: 'en_camino' });
        ...
        if (dist <= UMBRAL_LLEGADA_KM) {
          llegadaProcesandoRef.current = true;
          await base44.entities.Viaje.update(viaje.id, { estado: 'completado' });
          await base44.entities.Camion.update(miCamion.id, { estado: 'disponible' });
          llegadaProcesandoRef.current = false;
          setViajeActivo(null);
          cargarDatos();
        }
  ...
  const cargarDatos = async () => {
    try {
      const [camionesData, configsData, solicitudesData] = await Promise.all([
        base44.entities.Camion.filter({ user_id: user.id }),
        base44.entities.Configuracion.list(),
        base44.entities.Viaje.filter({ estado: 'solicitado' }),
      ]);
      ...
      if (camion) {
        const misViajes = await base44.entities.Viaje.filter({ camion_id: camion.id }, '-created_date', 10);
  ...
  const handleAceptar = async (solicitud) => {
    ...
    try {
      await base44.entities.Viaje.update(solicitud.id, {
        camion_id: miCamion.id,
        chofer_id: user.id,
        chofer_nombre: user?.full_name || miCamion.chofer_nombre,
        estado: 'aceptado',
      });
      await base44.entities.Camion.update(miCamion.id, { estado: 'ocupado' });
