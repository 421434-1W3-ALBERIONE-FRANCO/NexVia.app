...
export default function Admin() {
  ...
  const guardarConfig = async () => {
    ...
    try {
      ...
      if (config.id) {
        await base44.entities.Configuracion.update(config.id, data);
      } else {
        const created = await base44.entities.Configuracion.create(data);
        setConfig(created);
      }
      setGuardadoOk(true);
  ...
  const eliminarCamion = async (id) => {
    await base44.entities.Camion.delete(id);
    cargar();
  };
