...
export default function UsuariosSection({ camiones, onCamionAsignado }) {
  ...
  const cambiarRol = async (userId, nuevoRol) => {
    await base44.entities.User.update(userId, { role: nuevoRol });
  ...
  const asignarCamion = async (userId, camionId) => {
    ...
    await base44.entities.Camion.update(camionId, {
      user_id: userId,
  ...
  const invitar = async () => {
    ...
    setInvitando(true);
    try {
      await base44.users.inviteUser(invitarEmail, invitarRol);
      setInvitarEmail('');
